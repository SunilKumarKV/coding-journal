...
async function updateProblemVerification(problemJsonPath, verified) {
  const problem = normalizeProblemRecord(await readJson(problemJsonPath));
...
async function runValidationForDirectory(problemDir) {
  ...
  try {
    const [rawProblem, testsFile] = await Promise.all([readJson(problemJsonPath), readJson(testsPath)]);
    const problem = normalizeProblemRecord(rawProblem, path.basename(path.dirname(problemDir)));

...
import { readJson, resolvePaths, resolveRootDir, writeJson } from "./journal.js";

async function loadProjectsWithFallback(rootDir, options = {}) {
  try {
    return await fetchPublicProjects({ rootDir, fetchImpl: options.fetchImpl });
  } catch {
    return readJson(path.join(resolvePaths({ rootDir }).dataDir, "projects.json")).catch(() => []);
  }
}
...
export async function buildData(options = {}) {
  ...
  const [journal, projects] = await Promise.all([
    ...
    loadProjectsWithFallback(rootDir, options)
  ]);

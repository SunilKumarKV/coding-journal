    ...
    normalizedProblems.push(
      normalizeProblem({
        ...
        submissions: [
          {
            language: null,
            submittedAt: submission.timestamp ? new Date(Number(submission.timestamp) * 1000).toISOString() : null,
            source: "leetcode",
            codeAvailable: false,
            filename: null
          }
  ...
  return {
    ...
    warning: [
      ...
      normalizedProblems.length > 0
        ? "Accepted code is not publicly available. Use cj import-submission to attach your accepted code."
        : null
    ]
      .filter(Boolean)
      .join("\n") || null

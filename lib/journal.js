...
export function normalizeStoredPlatform(inputPlatform, fallbackPlatform = null) {
  const value = String(inputPlatform ?? "").trim().toLowerCase();

  if (PLATFORM_CONFIG[value]) {
    return value;
  }

  for (const [platformKey, config] of Object.entries(PLATFORM_CONFIG)) {
    if (config.label.toLowerCase() === value || config.directory.toLowerCase() === value) {
      return platformKey;
    }
  }

  if (fallbackPlatform && PLATFORM_CONFIG[String(fallbackPlatform).toLowerCase()]) {
...
export async function getProblemJsonDirectories(rootDir) {
  const directories = await collectProblemDirectories(rootDir);
  const matchingDirectories = [];

  for (const directory of directories) {
    if (await exists(path.join(directory, "problem.json"))) {
...
export function normalizeProblemRecord(problem = {}, fallbackPlatform = null) {
  return {
    ...problem,
    platform: normalizeStoredPlatform(problem.platform, fallbackPlatform)
  };
}
...
export async function upsertProblemJson(rootDir, platformKey, slug, updates) {
  const problemDir = getProblemDirectory(rootDir, platformKey, slug);
  const problemJsonPath = path.join(problemDir, "problem.json");
  const existing = (await exists(problemJsonPath))
    ? normalizeProblemRecord(await readJson(problemJsonPath), platformKey)
    : {};
  const merged = normalizeProblemRecord(
    {
      ...existing,
      ...updates,
...
export async function normalizeAllProblemJsonPlatforms(rootDir) {
  const directories = await collectProblemDirectories(rootDir);

  for (const directory of directories) {
    const platformKey = path.basename(path.dirname(directory));
    const problemJsonPath = path.join(directory, "problem.json");

    if (!(await exists(problemJsonPath))) {
      continue;
    }

    const problem = await readJson(problemJsonPath);
    const normalized = normalizeProblemRecord(problem, platformKey);

    if (JSON.stringify(problem) !== JSON.stringify(normalized)) {
      await writeJson(problemJsonPath, normalized);
    }
  }
}
